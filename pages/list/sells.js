...
Page({
  ...
  HZL_getEachRightItemToTop: function () {
    ...
    for (let i = 1; i < (constants.length + 1); i++) {
      totop += (RIGHT_BAR_HEIGHT + constants[i - 1].group.length * RIGHT_ITEM_HEIGHT)
  ...
  HZL_zero: function (e) {
    for (var i = 0; i < this.data.constants.length; i++) {
      for (var j = 0; j < this.data.constants[i].group.length; j++) {
        this.data.constants[i].group[j].num = 0
      }
    }
    this.setData({
      HZL_isCat: 0,
  ...
  HZL_my_jia: function (parentIndex, index) {
    ...
    var id = this.data.constants[parentIndex].group[index].productId;
    var productName = this.data.constants[parentIndex].group[index].productName;
    var num = ++this.data.constants[parentIndex].group[index].num;
    //弄一个元素判断会不会是重复的
    var mark = 'a' + index + 'b' + parentIndex + 'c' + '0' + 'd' + '0'
    var obj = { num: num, productName: productName, mark: mark, index: index, parentIndex: parentIndex };
  ...
  gotoInfo:function(e){
    console.log('gotoInfo:' + e.currentTarget.dataset.info.group_id + "," + e.currentTarget.dataset.info.group_name)
  },
  //封装减的方法
